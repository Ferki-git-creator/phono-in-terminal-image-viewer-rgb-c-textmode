...
#ifdef _WIN32
#include <windows.h>
// No conio.h or raw mode needed for non-interactive
#else
#include <sys/ioctl.h>
#include <termios.h> // For raw mode input (though not used in non-interactive mode anymore)
#endif

// STB Image defines for specific features/formats
#define STB_IMAGE_IMPLEMENTATION
// Removed STBI_NO_ defines to allow more formats by default (JPG, PNG, BMP, TGA, GIF, PSD, HDR, PIC, PNM)
// #define STBI_NO_PSD
// #define STBI_NO_GIF
// #define STBI_NO_PIC
// #define STBI_NO_PNM
// #define STBI_WINDOWS_UTF8 // For correct UTF-8 paths on Windows (if stbi_load supports it directly)
                          // Note: stbi_load expects char*, so full UTF-8 path support might need custom loader.
...
// Platform-specific terminal settings storage (kept for graceful exit handler)
#ifdef _WIN32
static DWORD s_original_windows_mode;
#else
...
void enable_raw_mode(void); // Kept for atexit handler, but not explicitly called in main
void disable_raw_mode(void); // Kept for atexit handler
 ...
 * Supports different color modes and includes a progress indicator.
 *
 ...
 */
void render_image(unsigned char *img_data, int width, int height, int channels, unsigned char bg_r, unsigned char bg_g, unsigned char bg_b) {
    // Clear the screen before rendering for a clean redraw
    printf("\033[H\033[J"); // ANSI escape code to clear screen and move cursor to home position
    ...
    // Progress bar setup (DISABLED to avoid visual artifacts in some terminals)
    // int show_progress = (height > 50); 
    int show_progress = 0; // Explicitly disable progress bar
    int progress_interval = height / 20; 
    if (progress_interval == 0) progress_interval = 1;
    
    if (show_progress) { // This block will now never execute
        fprintf(stderr, "Rendering: 0.0%%\r"); 
        fflush(stderr);
    }
    ...
    for (int y = 0; y < height; y++) {
        ...
        // Update progress bar (this block will now never execute)
        if (show_progress && ((y + 1) % progress_interval == 0 || (y + 1) == height)) {
            float percent = (float)(y + 1) / height * 100;
...
// Raw mode functions (kept for atexit handler, but not used directly in main anymore)
void enable_raw_mode(void) {
#ifdef _WIN32
    HANDLE hStdin = GetStdHandle(STD_INPUT_HANDLE);
    if (hStdin == INVALID_HANDLE_VALUE) {
        LOG_ERROR("%s", "Failed to get standard input handle.");
        return;
    }
    if (!GetConsoleMode(hStdin, &s_original_windows_mode)) {
        LOG_ERROR("Failed to get console mode: %lu", GetLastError());
        return;
    }
    DWORD new_mode = s_original_windows_mode & ~(ENABLE_ECHO_INPUT | ENABLE_LINE_INPUT);
    if (!SetConsoleMode(hStdin, new_mode)) {
        LOG_ERROR("Failed to set console mode: %lu", GetLastError());
        SetConsoleMode(hStdin, s_original_windows_mode);
    }
#else
    if (tcgetattr(STDIN_FILENO, &s_original_termios) == -1) {
        LOG_ERROR("%s", "Failed to get terminal attributes (tcgetattr).");
        return;
    }
    struct termios raw = s_original_termios;
    raw.c_lflag &= ~(ECHO | ICANON);
    raw.c_cc[VMIN] = 0;
    raw.c_cc[VTIME] = 0; // Non-blocking read
    if (tcsetattr(STDIN_FILENO, TCSAFLUSH, &raw) == -1) {
        LOG_ERROR("%s", "Failed to set terminal attributes (tcsetattr).");
...
void disable_raw_mode(void) {
    ...
    HANDLE hStdin = GetStdHandle(STD_INPUT_HANDLE);
    if (hStdin == INVALID_HANDLE_VALUE) {
        LOG_ERROR("%s", "Failed to get standard input handle for disable_raw_mode.");
        return;
    }
    if (!SetConsoleMode(hStdin, s_original_windows_mode)) {
        LOG_ERROR("Failed to restore terminal attributes (SetConsoleMode): %lu", GetLastError());
    }
#else
    if (tcsetattr(STDIN_FILENO, TCSAFLUSH, &s_original_termios) == -1) {
        LOG_ERROR("%s", "Failed to restore terminal attributes (tcsetattr).");
    }
#endif
    // Restore cursor visibility and clear progress bar line on exit
    printf("\033[?25h"); // Show cursor
    fprintf(stderr, "\r\033[K"); // Clear progress bar line
    fflush(stderr);
}
...
void handle_signal(int sig) {
    ...
    // disable_raw_mode(); // atexit will call this for normal exit path, but for signals we need it now.
                        // However, since we removed interactive mode, raw mode is not enabled by default.
                        // So, this call is mostly for cleanup consistency if raw mode was ever enabled.
    ...
    printf("\nExiting PIT gracefully.\n");
    exit(1); // Exit with an error code to indicate abnormal termination
}


// Cache functions (kept for future expansion, not used in single render mode)
unsigned char* get_cached_image(int width, int height) { 
    (void)width; // Suppress unused parameter warning
    (void)height; // Suppress unused parameter warning
    return NULL; 
}
void add_to_cache(int width, int height, unsigned char* data) { (void)width; (void)height; (void)data; }


/**
 * @brief Frees all memory allocated for the image cache.
 */
void free_image_cache(void) {
    if (s_image_cache) {
...
int main(int argc, char **argv) {
    ...
    // These will clean up resources even if the program is interrupted (e.g., Ctrl+C)
    signal(SIGINT, handle_signal);  // Ctrl+C
    signal(SIGTERM, handle_signal); // Termination signal
    atexit(disable_raw_mode); // Ensure terminal is restored on normal exit
